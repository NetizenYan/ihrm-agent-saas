...
public class SalaryArchiveDetail implements Serializable {
	...
	public void setSocialInfo(ArchiveDetail socialInfo) {
		if(socialInfo != null) {
			...
			this.providentFundIndividual = BigDecimal.ZERO;
			this.providentFundEnterprises = BigDecimal.ZERO;
			this.socialSecurityEnterprise = BigDecimal.ZERO;
			this.socialSecurityIndividual = BigDecimal.ZERO;
			//社保合计
			this.socialSecurityProvidentFundEnterprises =  BigDecimal.ZERO;
		}
	}
	...
	public void setAtteInfo(ArchiveMonthlyInfo atteInfo) {
		if(atteInfo == null) {
			this.officialSalaryDays = new BigDecimal("21.75");
		}else{
			//员工考勤天数
