...
//	}
	...
	@RequestMapping(value = "/list", method = RequestMethod.POST)
	public Result list(@RequestBody SearchListVo searchListVo) throws Exception {
		...
		Page<UserSocialSecurityItem> itemPage = userSocialService.findAll(searchListVo.getPage(), searchListVo.getPageSize(), companyId, searchListVo.getDepartmentChecks(), searchListVo.getSocialSecurityChecks(), searchListVo.getProvidentFundChecks());
		List<UserSocialSecurityItemVo> voList = new ArrayList<>();
		if (itemPage.getContent() != null) {
			for (UserSocialSecurityItem item : itemPage.getContent()) {
				UserSocialSecurityItemVo vo = new UserSocialSecurityItemVo();
				BeanUtils.copyProperties(item, vo);
				vo.setLeaveTime(vo.getLeaveTime());
				vo.setTimeOfEntry(DateUtil.parseDate2String(item.getTimeOfEntry(), "yyyy-MM-dd"));
				voList.add(vo);
			}

		}
		PageResult pageResult = new PageResult(itemPage.getTotalElements(), voList);
