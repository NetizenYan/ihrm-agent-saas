    ...
    <div class="contLeft">
      <div class="topTit">
        ...
        <div class="info">
          ...
          <p class="time">入职时间:  {{user.timeOfEntry | transformDateFormat('YYYY-MM-DD')}}   最新工资： {{formLabelAlign.currentPostWage + formLabelAlign.currentBasicSalary}} <ul><span class='more'>?</span><li>员工所有调薪后的基本工资、岗位工资合计</li></ul></p>
      ...
      <div>
        ...
        <div class="fund">
          ...
          <div class="table">
            ...
            <div class="tabRow"><div>缴费合计</div><div> </div><div>{{socialData.socialSecurityEnterprise + socialData.providentFundEnterprises}}</div><div> {{socialData.socialSecurityIndividual + socialData.providentFundIndividual}} </div></div>
          </div>
        </div>
        <div class="formTable">
          <el-form :label-position="'left'" label-width="180px" :model="formLabelAlign">
            <el-form-item label="实际出勤天数（正式）：" style="width: 50%;">
              <el-input v-model="atteData.actualAtteOfficialDays" placeholder="0" :disabled="true"></el-input>
            </el-form-item>
            <el-form-item label="计薪天数（正式）："  style="width: 50%;">
              <el-input v-model="atteData.workingDays" placeholder="0" :disabled="true"></el-input>
            </el-form-item>
          </el-form>
        </div>
      </div>
...
<script>
  import { getSalary,getSettings } from '@/api/hrm/salarysApi'
  import  { detail } from '@/api/base/users'
 import  { historysData } from '@/api/hrm/socialSecuritys'
 import  { atteArchiveDetail } from '@/api/hrm/attendances'
  ...
  export default {
    ...
    methods: {
      async init () {
        ...
        await detail({id:userId}).then(res=> {
          this.user = res.data.data
        })

        await atteArchiveDetail({userId:userId,yearMonth:yearMonth}).then(res=> {
          this.atteData = res.data.data
          getSettings().then(res1=> {
            let setting = res1.data.data;
            this.paymentData.p1=this.calMoney(setting.housingSubsidyScheme,setting.housingSubsidyAmount,this.atteData.actualAtteOfficialDays);
            this.paymentData.p2=this.calMoney(setting.transportationSubsidyScheme,setting.transportationSubsidyAmount,this.atteData.actualAtteOfficialDays);
            this.paymentData.p3=this.calMoney(setting.communicationSubsidyScheme,setting.communicationSubsidyAmount,this.atteData.actualAtteOfficialDays);
            this.paymentData.p4=this.calMoney(setting.lunchAllowanceScheme,setting.lunchAllowanceAmount,this.atteData.actualAtteOfficialDays);
          })
        })

        const { data: detailRes } = await getSalary({userId,yearMonth});
        
        if(detailRes.success == true){
          this.formLabelAlign = detailRes.data
        }
              
        await historysData({userId:userId,yearMonth:yearMonth}).then(res=> {
