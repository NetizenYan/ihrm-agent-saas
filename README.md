...
> 更完整的 Multi-Agent **v1 架构 / 设计文档**已发布于 [docs/agent/v1/](docs/agent/v1/)，索引见 [docs/agent/README.md](docs/agent/README.md) 与 [docs/agent/v1/README.md](docs/agent/v1/README.md)。

* v1 是**设计 / 提议文档，不是已实现的运行时**：仓库中不存在 Agent 运行时，也没有 Agent / Memory / 知识库 / MCP / Skills / token 成本相关数据库表。
* 当前实施优先级是**先稳定传统 HR SaaS 后端 / 前端 / 数据库**（`backend-modern` + `frontend-vue3`）。
* `backend-modern`（Spring Boot）始终是权威的 HR 业务系统。
* Agent 运行时、记忆、MCP、企业知识库与 token 成本运行时均为**提议中的未来工作**，尚未实现。
